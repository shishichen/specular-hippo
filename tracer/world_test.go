...
func TestWorld_ColorAt(t *testing.T) {
	...
	tests := []struct {
		...
		{"case1", NewDefaultWorld(), args{NewRay(NewPoint(0.0, 0.0, -5.0), NewVector(0.0, 1.0, 0.0))}, NewColor(0.0, 0.0, 0.0)},
		{"case2", NewDefaultWorld(), args{NewRay(NewPoint(0.0, 0.0, -5.0), NewVector(0.0, 0.0, 1.0))},
			NewColor(0.38066, 0.47583, 0.2855)},
		{"case3", NewWorld(
			Shapes{NewSphere().WithMaterial(NewMaterial(NewColor(0.8, 1.0, 0.6), 0.1, 0.7, 0.2, 200.0)),
				NewSphere().WithTransform(NewScale(0.5, 0.5, 0.5))},
			Lights{NewLight(NewPoint(0.0, 0.25, 0.0), NewColor(1.0, 1.0, 1.0))}),
			args{NewRay(NewPoint(0.0, 0.0, 0.0), NewVector(0.0, 0.0, 1.0))}, NewColor(0.90498, 0.90498, 0.90498)},
		{"case4", NewWorld(
			Shapes{NewSphere().WithMaterial(NewMaterial(NewColor(0.8, 1.0, 0.6), 1.0, 0.7, 0.2, 200.0)),
				NewSphere().WithTransform(NewScale(0.5, 0.5, 0.5)).
